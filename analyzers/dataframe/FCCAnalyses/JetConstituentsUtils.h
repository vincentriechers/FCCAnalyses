...
namespace FCCAnalyses {
  namespace JetConstituentsUtils {
    ...
    /// as get_PIDs_cluster, but returns a truth origin label from the MC parent
    /// ancestry (2 primary, 3 from b, 4 from b via c, 5 from c, 6 from tau,
    ///  7 other secondary: strange LLP decay (K_S0/Lambda/hyperon) or no MC
    ///  association)
    ...
    /// as get_PIDs_cluster, but returns a truth vertex index from clustering
    /// the matched MC production vertices (0 = largest cluster, 1,2,... =
    /// others, -1 = no MC association)
    rv::RVec<FCCAnalysesJetConstituentsData>
    ...
    /// per-jet PDG of the angularly matched production-flavour heavy hadron (0
    /// if none)
    rv::RVec<float>
    ...
    /// per-jet pT of the angularly matched gen jet, clustered from stable MC
    /// particles (-1 if none)
    rv::RVec<float>
